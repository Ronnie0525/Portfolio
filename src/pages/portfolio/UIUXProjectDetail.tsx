...
const UIUXProjectDetail = () => {
  ...
  return (
    <Layout>
      <SEOHead
        title={`${project.name} â€” UI/UX Case Study`}
        description={`${project.tagline}. A UI/UX design case study by Ronnie Balonon Jr. â€” freelance product designer for startups and brands worldwide.`}
      ...
      {/* Hero */}
      <section className="hero-dark relative min-h-[50vh] flex items-end overflow-hidden" style={{ background: `linear-gradient(135deg, ${project.accentFrom}20, #ffffff 60%)` }}>
        <div className="relative z-10 w-full max-w-[1280px] mx-auto px-6 pb-16 pt-32">
          ...
          <motion.div initial="hidden" animate="visible" variants={fadeIn} className="space-y-6 max-w-3xl">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-2xl flex items-center justify-center border border-black/20" style={{ background: `${project.accentFrom}30` }}>
                <Icon className="w-6 h-6" style={{ color: project.accentFrom }} />
              </div>
              <span className="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-[0.15em] border border-black/20" style={{ background: project.badgeColor, color: project.badgeTextColor }}>{project.badge}</span>
            </div>
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              {[{ icon: User, text: "Role: UI/UX Designer" },{ icon: Calendar, text: `Duration: ${project.duration}` },{ icon: Wrench, text: `Tools: ${project.tools}` },{ icon: Clock, text: `Year: ${project.year}` }].map(m => (
                <div key={m.text} className="flex items-center gap-2 px-4 py-2 rounded-full bg-black/10 backdrop-blur-md border border-black/20 text-sm text-black/80">
      ...
      <section className="py-24">
        <div className="max-w-[1280px] mx-auto px-6">
          <div className="grid lg:grid-cols-5 gap-8">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="lg:col-span-3 rounded-2xl border border-black/15 backdrop-blur-xl p-8 space-y-4" style={{ background: "rgba(255,255,255,0.03)" }}>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="lg:col-span-2 rounded-2xl border border-black/15 backdrop-blur-xl p-8 space-y-4" style={{ background: "rgba(255,255,255,0.03)" }}>
      ...
      <section className="py-24">
        <div className="max-w-[1280px] mx-auto px-6 space-y-12">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="group cursor-pointer" onClick={() => navigate(`/portfolio/uiux/${nextProject.id}`)}>
            <div className="rounded-2xl border border-black/15 overflow-hidden backdrop-blur-xl transition-all duration-500 group-hover:border-black/20" style={{ background: "rgba(255,255,255,0.03)" }}>
