    ...
    >
      ...
      <div className={`relative w-9 h-9 rounded-lg flex items-center justify-center transition-all duration-300 flex-shrink-0 ${
        ...
        <item.icon className={`w-4 h-4 transition-all duration-300 ${
          isNew ? 'text-purple-300 group-hover:text-white' : 'text-white/40 group-hover:text-white'
        }`} />
      </div>
      <div className="min-w-0 flex-1 relative z-10">
        <div className="flex items-center gap-2">
          <p className="text-[13px] font-medium text-white/70 group-hover:text-white transition-colors duration-300">{item.name}</p>
          {isNew && (
            <span className="text-[8px] font-bold uppercase tracking-widest bg-gradient-to-r from-purple-500 to-fuchsia-500 text-white px-1.5 py-0.5 rounded-full leading-none">
              NEW
            </span>
          )}
        </div>
        <p className="text-[11px] text-white/25 group-hover:text-white/45 leading-tight transition-colors">{item.description}</p>
      </div>
              ...
              <div className="px-6 py-3 border-t border-white/[0.04]" style={{ background: "rgba(168,85,247,0.03)" }}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-6 text-[11px] text-white/30">
                    <span><strong className="text-white/60 font-semibold">4</strong> Core Skills</span>
                  ...
                  <div className="flex items-center gap-3">
                    <Link to="/proposals" onClick={onClose} className="flex items-center gap-1.5 text-[11px] font-medium text-white/35 hover:text-white/70 transition-colors">
