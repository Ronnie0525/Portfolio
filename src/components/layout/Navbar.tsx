                          ...
                          <div className="relative p-2 rounded-xl overflow-hidden" style={{ 
                            ...
                            {item.dropdown.map((subItem, i) => (
                              <motion.div key={subItem.name} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03, duration: 0.2 }}>
                                <Link 
                                  to={subItem.path} 
                                  className="flex items-center gap-3 px-3 py-2.5 text-sm text-black/60 hover:text-black rounded-lg transition-all duration-300 group relative overflow-hidden hover:bg-white/[0.04]"
                                >
                                  ...
                                  {subItem.icon && (
                                    <div className="w-7 h-7 rounded-md bg-white/[0.04] flex items-center justify-center group-hover:bg-gradient-to-br group-hover:from-purple-500/80 group-hover:to-fuchsia-500/80 group-hover:shadow-[0_0_12px_rgba(168,85,247,0.3)] transition-all duration-300">
                                      <subItem.icon className="w-3.5 h-3.5 text-black/35 group-hover:text-black transition-colors duration-300" />
                                    </div>
                                  )}
            ...
            <div className="container-custom py-6 mx-[10px]">
              ...
              <div className="flex items-center gap-4 pb-6 mb-6 border-b border-white/[0.06]">
                ...
                <div>
                  ...
                  <p className="text-xs text-black/35">Designer · AI · Dubai</p>
                </div>
              </div>
              ...
              <nav className="space-y-1 mb-6" aria-label="Mobile navigation">
                ...
                <div>
                  <button
                    ...
                    className="flex items-center justify-between w-full px-4 py-3 rounded-lg text-base font-semibold text-black hover:bg-white/[0.03] transition-colors"
                  >
                    Services
                    <ChevronDown className={`w-4 h-4 text-black/30 transition-transform duration-200 ${activeDropdown === "mobile-services" ? "rotate-180" : ""}`} />
                  </button>
                  <AnimatePresence>
                    {activeDropdown === "mobile-services" && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                        ...
                        {mobileServiceItems.map(item => (
                          <Link key={item.path} to={item.path} onClick={() => setIsOpen(false)} className="flex items-center gap-3 px-6 py-2.5 text-sm text-black/50 hover:text-black hover:bg-white/[0.03] transition-all">
                ...
                {/* Portfolio */}
                <Link to="/portfolio" onClick={() => setIsOpen(false)} className="block px-4 py-3 rounded-lg text-base font-semibold text-black hover:bg-white/[0.03] transition-colors">
                  Portfolio
                </Link>
                ...
                {navItems.filter(i => i.name === "Company").map(item => (
                  <div key={item.name}>
                    ...
                    >
                      {item.name}
                      <ChevronDown className={`w-4 h-4 text-black/30 transition-transform duration-200 ${activeDropdown === "mobile-expertise" ? "rotate-180" : ""}`} />
                    </button>
                    <AnimatePresence>
                      {activeDropdown === "mobile-expertise" && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                          {item.dropdown?.map(subItem => (
                            <Link key={subItem.name} to={subItem.path} onClick={() => setIsOpen(false)} className="flex items-center gap-3 px-6 py-2.5 text-sm text-black/50 hover:text-black hover:bg-white/[0.03] transition-all">
                ...
                <div>
                  ...
                  >
                    Digital Platform
                    <ChevronDown className={`w-4 h-4 text-black/30 transition-transform duration-200 ${activeDropdown === "mobile-platforms" ? "rotate-180" : ""}`} />
                  </button>
                  <AnimatePresence>
                    {activeDropdown === "mobile-platforms" && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                        ...
                        {mobilePlatformItems.map(item => (
                          <a key={item.name} href={item.url} target="_blank" rel="noopener noreferrer" onClick={() => setIsOpen(false)} className="flex items-center gap-3 px-6 py-2.5 text-sm text-black/50 hover:text-black hover:bg-white/[0.03] transition-all">
                            <item.icon className="w-4 h-4 text-purple-mid/40" />
                            {item.name}
                          </a>
                        ))}
                ...
                {navItems.filter(i => i.name === "Client Web Project").map(item => (
                  <div key={item.name}>
                    ...
                    >
                      {item.name}
                      <ChevronDown className={`w-4 h-4 text-black/30 transition-transform duration-200 ${activeDropdown === "mobile-clients" ? "rotate-180" : ""}`} />
                    </button>
                    <AnimatePresence>
                      {activeDropdown === "mobile-clients" && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                          {item.externalDropdown?.map((subItem: ExternalItem) => (
                            <a key={subItem.name} href={subItem.url} target="_blank" rel="noopener noreferrer" onClick={() => setIsOpen(false)} className="flex items-center gap-3 px-6 py-2.5 text-sm text-black/50 hover:text-black hover:bg-white/[0.03] transition-all">
                              <subItem.icon className="w-4 h-4 text-purple-mid/40" />
                              {subItem.name}
                            </a>
                          ))}
              ...
              <div className="mt-6 pt-6 border-t border-white/[0.06] space-y-2">
                <a href="mailto:ronniebalonon1996@gmail.com" className="flex items-center gap-2 text-sm text-black/35 hover:text-purple-glow">
                  <Mail className="w-4 h-4" />
                  ronniebalonon1996@gmail.com
                </a>
                <a href="https://wa.me/971543763091" className="flex items-center gap-2 text-sm text-black/35 hover:text-purple-glow">
