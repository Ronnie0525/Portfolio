...
const TabBar = ({ active = 0 }: { active?: number }) => (
  <div className="flex items-center justify-around px-4 py-3 border-t border-black/5 mt-auto">
    {[Home, Search, Heart, ShoppingBag, User].map((Icon, i) => (
      <div key={i} className="flex flex-col items-center gap-1"><Icon className={`w-5 h-5 ${i === active ? "text-black" : "text-black/20"}`} />{i === active && <div className="w-1.5 h-1.5 rounded-full bg-black" />}</div>
    ))}
  </div>
);
...
export const ThreadsHome = () => (
  <Phone><div className="flex-1 flex flex-col px-5 py-2">
    <div className="flex items-center justify-between mb-3"><span className="text-base font-bold text-black tracking-[0.2em]" style={{ fontFamily: "serif" }}>THREADS</span><div className="flex items-center gap-3"><Search className="w-5 h-5 text-black/40" /><Heart className="w-5 h-5 text-black/40" /><div className="relative"><ShoppingBag className="w-5 h-5 text-black/40" /><div className="absolute -top-1 -right-1.5 w-3.5 h-3.5 rounded-full bg-red-500 flex items-center justify-center text-[7px] text-white font-bold">2</div></div></div></div>
    <div className="rounded-xl px-4 py-3 mb-3" style={{ background: "linear-gradient(135deg, #FCE4EC, #FEF3C7)" }}><div className="text-sm font-bold text-black/80">Spring Collection</div><div className="text-xs text-black/50 mb-1.5">New season, new looks</div><div className="px-3 py-1 rounded-full bg-black text-white text-[10px] font-bold w-fit">Shop Now →</div></div>
...
export const ThreadsProduct = () => (
  <Phone><div className="flex-1 flex flex-col px-5 py-2">
    ...
    <div className="flex gap-1.5 mb-3">{["S","M","L","XL"].map((s,i)=><div key={s} className={`px-3 py-1.5 rounded-lg text-[10px] font-bold ${i===1?"bg-black text-white":"bg-black/5 text-black/50"}`}>{s}</div>)}</div>
    <div className="flex items-center gap-1 text-[10px] text-black/40 mb-3"><Ruler className="w-3 h-3"/> Size Guide →</div>
    <div className="py-3 rounded-xl bg-black text-white text-center text-xs font-bold mt-auto">Add to Bag</div>
...
export const ThreadsSizeGuide = () => (
  <Phone><div className="flex-1 flex flex-col px-5 py-2">
    ...
    <div className="flex justify-center mb-4"><div className="w-16 h-28 rounded-xl bg-black/5 flex items-center justify-center"><User className="w-10 h-10 text-black/20"/></div></div>
    {["Height (cm)","Weight (kg)","Chest (cm)"].map(f=>(<div key={f} className="mb-3"><div className="text-[10px] text-black/40 mb-1">{f}</div><div className="h-2 rounded-full bg-black/5"><div className="h-full rounded-full bg-black/20" style={{width:"60%"}}/></div></div>))}
    ...
    <div className="py-3 rounded-xl bg-emerald-500 text-white text-center text-xs font-bold mt-auto">Apply</div>
  </div><TabBar/></Phone>
);
...
export const ThreadsCart = () => (
  <Phone><div className="flex-1 flex flex-col px-5 py-2">
    ...
    <div className="space-y-2 flex-1">{[{name:"Wool Blazer",size:"M",color:"Navy",price:"$129"},{name:"Linen Shirt",size:"M",color:"White",price:"$89"},{name:"Silk Scarf",size:"OS",color:"Beige",price:"$129"}].map(item=>(<div key={item.name} className="flex items-start gap-3 p-2 rounded-xl bg-black/[0.02] border border-black/5"><div className="w-12 h-14 rounded-lg flex-shrink-0 overflow-hidden bg-[#f5f5f0]"><img src={productImages[item.name]} alt={item.name} className="w-full h-full object-cover" /></div><div className="flex-1 min-w-0"><div className="text-xs font-semibold text-black">{item.name}</div><div className="flex gap-1 mt-0.5"><span className="text-[8px] px-1.5 py-0.5 rounded bg-black/5 text-black/50">{item.size}</span><span className="text-[8px] px-1.5 py-0.5 rounded bg-black/5 text-black/50">{item.color}</span></div><div className="flex items-center gap-2 mt-1.5"><div className="flex items-center gap-1.5 border border-black/10 rounded-lg px-1"><Minus className="w-3 h-3 text-black/30"/><span className="text-[10px] font-bold text-black px-1">1</span><Plus className="w-3 h-3 text-black/30"/></div><span className="text-xs font-bold text-black ml-auto">{item.price}</span></div></div><X className="w-3.5 h-3.5 text-black/20 flex-shrink-0"/></div>))}</div>
    ...
    <div className="py-3 rounded-xl bg-black text-white text-center text-xs font-bold mt-3">Checkout — $347</div>
  </div><TabBar active={3}/></Phone>
);

export const ThreadsCheckout = () => (
  <Phone><div className="flex-1 flex flex-col px-5 py-2">
    <div className="flex items-center justify-center gap-3 mb-4">{[{n:"1",l:"Shipping",done:true},{n:"2",l:"Payment",active:true},{n:"3",l:"Confirm"}].map((s,i)=>(<div key={s.n} className="flex items-center gap-1.5">{i>0&&<div className="w-6 h-px bg-black/10"/>}<div className={`w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-bold ${s.done?"bg-emerald-500 text-white":s.active?"bg-black text-white":"bg-black/5 text-black/30"}`}>{s.done?"✓":s.n}</div><span className="text-[9px] text-black/50">{s.l}</span></div>))}</div>
    <div className="space-y-3 flex-1">
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl border border-black/10"><div className="w-4 h-4 rounded-full border-2 border-black flex items-center justify-center"><div className="w-2 h-2 rounded-full bg-black"/></div><span className="text-xs text-black">••4242 Visa</span></div>
      <div className="flex gap-2"><div className="flex-1 py-2.5 rounded-xl bg-black text-white text-center text-[10px] font-bold"> Pay</div><div className="flex-1 py-2.5 rounded-xl bg-black text-white text-center text-[10px] font-bold">G Pay</div></div>
    </div>
    <div className="rounded-xl p-3 bg-black/[0.02] border border-black/5 mt-2"><div className="flex justify-between text-xs"><span className="text-black/50">Total</span><span className="font-bold text-black">$347.00</span></div></div>
  </div><TabBar active={3}/></Phone>
);
