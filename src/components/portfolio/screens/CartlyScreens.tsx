...
export const CartlySearch = () => (
  ...
  <div className="flex-1 flex flex-col px-4 py-2 bg-white">
    ...
    <div className="space-y-2 flex-1">{[{name:"ProSound X7",price:"$79.99",badge:"Cartly's Choice"},{name:"BeatWave Pro",price:"$54.99"},{name:"AudioMax Elite",price:"$89.99"}].map(p=>(<div key={p.name} className="flex items-center gap-2.5 p-2 rounded-xl border border-gray-100"><div className="w-14 h-14 rounded-lg flex-shrink-0" style={{background:"linear-gradient(135deg,#DBEAFE,#93C5FD)"}}/><div className="flex-1 min-w-0"><div className="text-[10px] font-semibold text-gray-800">{p.name}</div><div className="flex items-center gap-0.5"><Star className="w-2 h-2 text-amber-400 fill-amber-400"/><span className="text-[8px] text-gray-400">4.8</span></div><div className="text-xs font-bold text-gray-900 mt-0.5">{p.price}</div>{p.badge&&<span className="text-[7px] px-1 py-0.5 rounded bg-teal-100 text-teal-700 font-bold">{p.badge}</span>}</div><div className="px-2 py-1 rounded-lg bg-orange-500 text-black text-[8px] font-bold">Add</div></div>))}</div>
...
export const CartlyProduct = () => (
  <Phone><div className="flex-1 flex flex-col bg-white">
    ...
    <div className="flex-1 px-4 py-3"><div className="text-sm font-bold text-gray-900">ProSound X7</div><div className="flex items-center gap-1 my-1">{[1,2,3,4,5].map(s=><Star key={s} className="w-3 h-3 text-amber-400 fill-amber-400"/>)}<span className="text-[10px] text-gray-400">4.8 (3,421)</span></div><div className="flex items-baseline gap-2 mb-2"><span className="text-lg font-bold text-gray-900">$79.99</span><span className="text-xs text-gray-400 line-through">$129.99</span><span className="text-[10px] text-red-500 font-bold">-38%</span></div><div className="flex gap-2 mb-2">{[{c:"#f0f0f0",a:true},{c:"#F5F5F0"},{c:"#1E3A5F"}].map((d,i)=><div key={i} className={`w-6 h-6 rounded-full ${d.a?"ring-2 ring-orange-500 ring-offset-1":""}`} style={{background:d.c}}/>)}</div><div className="flex gap-1.5 mb-3">{["40hr","ANC","BT5.3"].map(s=><span key={s} className="px-2 py-1 rounded-lg bg-gray-100 text-[9px] font-bold text-gray-600">{s}</span>)}</div><div className="flex gap-2 mb-2"><div className="flex-1 py-2.5 rounded-xl bg-orange-500 text-black text-center text-xs font-bold">Add to Cart</div><div className="flex-1 py-2.5 rounded-xl border border-gray-200 text-gray-700 text-center text-xs font-bold">Buy Now</div></div><div className="flex items-center gap-1.5 text-[10px] text-gray-500"><ShieldCheck className="w-3.5 h-3.5 text-emerald-500"/> TechAudio · 98% positive</div></div>
  </div><TabBar/></Phone>
);
...
export const CartlyCompare = () => (
  <Phone><div className="flex-1 flex flex-col px-4 py-2 bg-white">
    ...
    <div className="py-2.5 rounded-xl bg-orange-500 text-black text-center text-xs font-bold">Choose Winner</div>
  </div><TabBar/></Phone>
);
...
export const CartlyCart = () => (
  <Phone><div className="flex-1 flex flex-col px-4 py-2 bg-white">
    ...
    <div className="py-2.5 rounded-xl bg-orange-500 text-black text-center text-xs font-bold">Checkout — $187.96</div>
...
export const CartlyTracking = () => (
  <Phone><div className="flex-1 flex flex-col px-4 py-2 bg-white">
    ...
    <div className="flex items-center gap-3 my-4">{[{l:"Ordered",done:true},{l:"Packed",done:true},{l:"Shipped",active:true},{l:"Delivered"}].map((s,i)=>(<div key={s.l} className="flex-1 flex flex-col items-center gap-1"><div className={`w-7 h-7 rounded-full flex items-center justify-center text-[8px] font-bold ${s.done?"bg-emerald-500 text-black":s.active?"bg-orange-500 text-black animate-pulse":"bg-gray-100 text-gray-400"}`}>{s.done?"✓":s.active?<Package className="w-3 h-3"/>:(i+1)}</div><span className="text-[8px] text-gray-500">{s.l}</span></div>))}</div>
    ...
    <div className="py-2.5 rounded-xl bg-teal-500 text-black text-center text-xs font-bold mt-3">Track Package</div>
  </div><TabBar/></Phone>
);
