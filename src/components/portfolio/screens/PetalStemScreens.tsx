...
export const PetalDetail = () => (
  <Phone><div className="flex-1 flex flex-col px-5 py-2">
    ...
    <div className="flex gap-2 mb-2"><div className="flex-1 py-2 rounded-lg text-center text-[10px] font-bold bg-rose-500 text-black">Standard $85</div><div className="flex-1 py-2 rounded-lg text-center text-[10px] font-bold border border-rose-200 text-rose-500">Deluxe $120</div></div>
    ...
    <div className="py-3 rounded-xl bg-rose-500 text-black text-center text-xs font-bold mt-auto">Add to Cart — $85</div>
...
export const PetalMessage = () => (
  <Phone><div className="flex-1 flex flex-col px-5 py-2">
    ...
    <div className="flex gap-1.5 mb-3">{["Classic","Script","Handwritten"].map((f,i)=><div key={f} className={`px-2.5 py-1 rounded-lg text-[9px] font-bold ${i===0?"bg-rose-500 text-black":"bg-rose-50 text-rose-400"}`}>{f}</div>)}</div>
    ...
    <div className="py-3 rounded-xl bg-rose-500 text-black text-center text-xs font-bold mt-auto">Save</div>
  </div><TabBar/></Phone>
);
...
export const PetalDelivery = () => (
  <Phone><div className="flex-1 flex flex-col px-5 py-2">
    ...
    <div className="grid grid-cols-7 gap-1 mb-3">{["M","T","W","T","F","S","S"].map((d,i)=><div key={`${d}${i}`} className={`w-full aspect-square rounded-lg flex items-center justify-center text-[9px] font-bold ${i===3?"bg-rose-500 text-black":"bg-rose-50 text-rose-400"}`}>{i+3}</div>)}</div>
    <div className="space-y-1.5 mb-3">{[{l:"Morning 9-12",icon:Sun},{l:"Afternoon 12-4",icon:CloudSun,active:true},{l:"Evening 4-8",icon:Moon}].map(t=>(<div key={t.l} className={`flex items-center gap-2 px-3 py-2.5 rounded-xl ${t.active?"bg-rose-500 text-black":"bg-rose-50 text-rose-600"}`}><t.icon className="w-4 h-4"/><span className="text-[10px] font-medium">{t.l}</span></div>))}</div>
    ...
    <div className="py-3 rounded-xl bg-rose-500 text-black text-center text-xs font-bold mt-auto">Confirm</div>
  </div><TabBar/></Phone>
);
...
export const PetalTracking = () => (
  <Phone><div className="flex-1 flex flex-col px-5 py-2">
    ...
    <div className="flex items-center gap-3 mb-4">{[{l:"Arranged",done:true},{l:"Quality",done:true},{l:"Delivering",active:true},{l:"Delivered"}].map((s,i)=>(<div key={s.l} className="flex-1 flex flex-col items-center gap-1"><div className={`w-7 h-7 rounded-full flex items-center justify-center text-[8px] font-bold ${s.done?"bg-emerald-500 text-black":s.active?"bg-rose-500 text-black animate-pulse":"bg-rose-100 text-rose-300"}`}>{s.done?"✓":s.active?<Truck className="w-3 h-3"/>:(i+1)}</div><span className="text-[7px] text-rose-400">{s.l}</span></div>))}</div>
