...
const ThreadsMockup = () => (
  <div className="absolute inset-0 flex flex-col" style={{ background: "#FAF9F6" }}>
    ...
    <div className="flex items-center justify-between px-3 py-1.5">
      ...
      <div className="flex items-center gap-2"><Search className="w-3 h-3 text-black/40" /><Heart className="w-3 h-3 text-black/40" /><div className="relative"><ShoppingBag className="w-3 h-3 text-black/40" /><div className="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-red-500 flex items-center justify-center text-[5px] text-white font-bold">2</div></div></div>
    </div>
    <div className="mx-3 rounded-lg px-2.5 py-2 mb-1.5" style={{ background: "linear-gradient(135deg, #FCE4EC, #FEF3C7)" }}>
      <div className="text-[9px] font-bold text-black/80">Spring '25</div><div className="text-[7px] text-black/50">New season, new looks</div>
      <div className="mt-1 px-2 py-0.5 rounded-full bg-black text-white text-[6px] font-bold w-fit">Shop Now</div>
    </div>
      ...
      <div className="grid grid-cols-2 gap-1.5">
        {[{ name: "Wool Blazer", price: "$129" }, { name: "Linen Shirt", price: "$89" }].map(p => (
          <div key={p.name} className="rounded-lg overflow-hidden border border-black/5">
            <div className="relative h-16" style={{ background: "linear-gradient(135deg, #D6D3D1, #A8A29E)" }}><Heart className="absolute top-1 right-1 w-2.5 h-2.5 text-black/30" /></div>
    ...
    <div className="mt-auto flex items-center justify-around px-3 py-1.5 border-t border-black/5">
      <div className="flex flex-col items-center gap-0.5"><Home className="w-3 h-3 text-black" /><div className="w-1 h-1 rounded-full bg-black" /></div>
      <Search className="w-3 h-3 text-black/20" /><Heart className="w-3 h-3 text-black/20" /><ShoppingBag className="w-3 h-3 text-black/20" /><User className="w-3 h-3 text-black/20" />
    </div>
  </div>
);
