...
const MobilixMockup = () => (
  <div className="absolute inset-0 flex flex-col" style={{ background: "#0A1628" }}>
    ...
    <div className="flex items-center justify-between px-3 py-1.5">
      ...
      <div className="flex items-center gap-2"><Search className="w-3 h-3 text-black/75" /><ShoppingCart className="w-3 h-3 text-black/75" /></div>
    </div>
    <div className="mx-3 rounded-xl p-2.5" style={{ background: "linear-gradient(135deg, #164E63, #0F172A)" }}>
      <div className="text-[9px] font-bold text-black">Find Your Phone</div><div className="text-[6px] text-black/80 mb-1.5">Personalized recommendations</div>
    ...
    <div className="flex-1 px-3 space-y-1">
      {[{name:"Galaxy Ultra Pro",rating:"4.7",price:"$999",badge:"New"},{name:"iPhone 16 Pro",rating:"4.8",price:"$1,099",badge:null},{name:"Pixel 9 Pro",rating:"4.5",price:"$899",badge:null}].map(p => (
        <div key={p.name} className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-black/5 border border-black/15">
          <Smartphone className="w-4 h-4 text-cyan-400/40" />
          <div className="flex-1 min-w-0"><div className="flex items-center gap-1"><span className="text-[7px] font-semibold text-black">{p.name}</span>{p.badge && <span className="text-[5px] px-1 py-0.5 rounded bg-cyan-400/20 text-cyan-300 font-bold">{p.badge}</span>}</div><div className="flex items-center gap-0.5"><Star className="w-2 h-2 text-amber-400 fill-amber-400" /><span className="text-[6px] text-black/75">{p.rating}</span></div></div>
    ...
    <div className="flex items-center justify-around px-3 py-1.5 border-t border-black/15">
      ...
      <Search className="w-3 h-3 text-black/65" /><GitCompare className="w-3 h-3 text-black/65" /><Heart className="w-3 h-3 text-black/65" /><User className="w-3 h-3 text-black/65" />
    </div>
  </div>
);
